...
void UCombatComponent::AttachActorToRightHand(AActor* ActorToAttach)
{
	...
	USkeletalMeshSocket const* HandSocket = Character->GetMesh()->GetSocketByName(FName(TEXT("RightHandSocket")));
	if (HandSocket)
	{
...
{
	if (Character == nullptr || Character->GetMesh() == nullptr || ActorToAttach == nullptr) return;
	USkeletalMeshSocket const* BackpackSocket = Character->GetMesh()->GetSocketByName(FName(TEXT("BackPackSocket")));
	if (BackpackSocket)
	{
...
void UCombatComponent::FireProjectileWeapon()
{
	if (EquippedWeapon)
	{
		HitTarget = EquippedWeapon->bUseScatter ? EquippedWeapon->TraceEndWhithScatter(HitTarget) : HitTarget;
		LocalFire(HitTarget);
...
{
	MultcastShotgunFire(TraceHitTargets);
}

void UCombatComponent::MultcastShotgunFire_Implementation(const TArray<FVector_NetQuantize>& TraceHitTargets)
{
	if (Character && Character->IsLocallyControlled()) return;
...
void UCombatComponent::OnRep_CombatState()
{
	...
	case ECombatState::ECS_ThrowingGrenade:
		if (Character && !Character->IsLocallyControlled())
		{
			Character->PlayThrowGrenadeMotag();
...
void UCombatComponent::JumpToShotgunEnd()
{
	...
	if (Character->GetReloadMotage() && AnimInstance)
	{
